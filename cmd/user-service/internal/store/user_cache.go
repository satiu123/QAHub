...
func (s *userCacheStore) CreateUser(ctx context.Context, user *model.User) (int64, error) {
	return s.next.CreateUser(ctx, user)
}

// UpdateUser 首先更新数据库，如果成功，则使缓存失效。
func (s *userCacheStore) UpdateUser(ctx context.Context, user *model.User) error {
	// 1. 首先更新数据库
...
func (s *userCacheStore) DeleteUser(ctx context.Context, id int64) error {
...
func (s *userCacheStore) GetUserByID(ctx context.Context, id int64) (*model.User, error) {
	key := userKey(id)
...
func (s *userCacheStore) GetUserByUsername(ctx context.Context, username string) (*model.User, error) {
	...
	// 2. 缓存未命中，从数据库获取
	user, err := s.next.GetUserByUsername(ctx, username)
	if err != nil {
		return nil, err
	}
...
func (s *userCacheStore) GetUserByEmail(ctx context.Context, email string) (*model.User, error) {
